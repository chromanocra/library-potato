...
class PenggunaController extends ResourceController
{
        ...
        if (empty($input)) {
            return $this->fail(
                [
                    "status" => 400,
                    "pesan" =>
                        "Data tidak boleh kosong. Kirim JSON dengan Content-Type: application/json.",
                ],
                400,
            );
        }
        ...
        if (isset($user["status"]) && $user["status"] === "inactive") {
            return $this->fail(
                [
                    "status" => 403,
                    "pesan" =>
                        "Akun Anda telah dinonaktifkan. Hubungi admin perpustakaan.",
                ],
                403,
            );
        }
    ...
    public function update($id = null)
    {
        ...
        return $this->fail(
            [
                "pesan" => "Gagal update data pengguna",
                "errors" => $model->errors(),
            ],
            500,
        );
    ...
    public function toggleSuspend($id = null)
    {
        ...
        if (($user["role"] ?? "user") === "admin") {
            return $this->fail(
                ["pesan" => "Akun admin tidak dapat dinonaktifkan."],
                400,
            );
        }
